...
function AddMenuModal({ meals, setMeals, pos }) {
  let { handleModal } = useContext(ModalContext);

  const { data: mealsData } = useGetData("/menu/all", "getallmenu", 50);
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    if (isIdInList(meals, values.menu)) {
      toast.error("menu deja exist");
      return;
    }

    const name = ListIngrednet.filter((ing) => ing.value == values.menu);
    console.log(name);

    meals[pos] = { id: values.menu, name: name[0].label };
    setMeals(meals);
    resetForm();
    handleModal();
  };
